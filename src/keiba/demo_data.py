...
def _synthesize_race(race_index: int, rng: random.Random) -> tuple[RaceCard, list[RaceResult], dict[str, list[PastPerformance]]]:
    ...
    strength = {e.horse_number: rng.random() / next(o.win_odds for o in odds_list if o.horse_number == e.horse_number) for e in entries}
    ...
    # 過去成績は「潜在的な強さ(strength上の順位)」に緩く相関させた合成データ
    # （実際の過去レース結果ではなく、動作確認用のダミー）。
    ability_rank = {horse: pos + 1 for pos, horse in enumerate(finish_order)}
...
def generate_backtest_cases(n_races: int = 20, seed: int = 42) -> list[BacktestCase]:
    ...
    for i in range(1, n_races + 1):
        ...
        scores = score_race(features)
        cases.append(BacktestCase(race_id=card.race.race_id, scores=scores, results=results, odds=card.odds))
    return cases
