...
| S6 | エラーメッセージ画面（**未設計**） | 各コマンド実行時、例外発生時 | なし（現状は例外処理自体が未実装） |
...
>
> **S6について**: 詳細は`05_処理フロー.md` UC6を参照。現状はエラーメッセージ画面という概念自体が存在せず、Pythonの生トレースバックがそのまま表示される。
...
### S6: エラーメッセージ画面（現状 vs あるべき設計）

現状（例: 存在しないrace-idを指定した場合）:
...
Traceback (most recent call last):
  File "...\cli.py", line 74, in <module>
    main()
  ...
ValueError: race_id='XXX' のサンプルデータが登録されていません
```

あるべき設計（未実装、`05_処理フロー.md` UC6参照）:

```
エラー: レースID 'XXX' のデータが見つかりません
```
（終了コード1）
